...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:variable name="doi">https://doi.org/10.6069/uwlib.55.a.3.6</xsl:variable>
        <xsl:variable name="dataDesc">https://doi.org/10.6069/uwlib.55.a#uwSemWeb</xsl:variable>
        <xsl:variable name="usw"
            select="document('file:////Users/theodore/Desktop/ldStage/datasets/uwSemWeb.rdf')"/>
        <xsl:variable name="name" select="$usw/rdf:RDF/rdf:Description[@rdf:about = $doi]/dct:title"/>
        <html xmlns="http://www.w3.org/1999/xhtml" version="XHTML+RDFa 1.1"
            xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
            xsi:schemaLocation="http://www.w3.org/1999/xhtml http://www.w3.org/MarkUp/SCHEMA/xhtml-rdfa-2.xsd"
            prefix="rdf:http://www.w3.org/1999/02/22-rdf-syntax-ns# dct:http://purl.org/dc/terms/ void:http://rdfs.org/ns/void# owl:http://www.w3.org/2002/07/owl# dc:http://purl.org/dc/elements/1.1/ edm:http://www.europeana.eu/schemas/edm/ rdfs:http://www.w3.org/2000/01/rdf-schema# foaf:http://xmlns.com/foaf/0.1/ dpla:http://dp.la/about/map/ skos:http://www.w3.org/2004/02/skos/core# bf:http://id.loc.gov/ontologies/bibframe/ ore:http://www.openarchives.org/ore/terms/ dcmitype:http://purl.org/dc/dcmitype/"  
            lang="en" xml:lang="en">
            <head>
                <title>
            ...
            <body>
                ...
                <h2>Links to Alternate Serializations for <xsl:value-of
                        select="$usw/rdf:RDF/rdf:Description[@rdf:about = $doi]/dct:title"/></h2>
                <ul>
                    <xsl:for-each
                        select="$usw/rdf:RDF/rdf:Description[@rdf:about = $doi]/dct:hasFormat">
                        <xsl:choose>
                            <xsl:when test="ends-with(@rdf:resource, '.nt')">
                                <li>
                ...
                <p>Version <xsl:value-of
                        select="$usw/rdf:RDF/rdf:Description[@rdf:about = $doi]/owl:version"/>
                    issued <xsl:value-of
                        select="$usw/rdf:RDF/rdf:Description[@rdf:about = $doi]/dct:issued"/></p>
                <!-- Table headline -->
